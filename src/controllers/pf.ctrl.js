...
app.controller(
  ...
  function (
    ...
    document.title = "PROFESSIONAL FEE | GMMRP";
    $scope.supplier = 0;
    ...
    $scope.sales_list = function (sid, startDate, endDate) {
      $rest
        .get(
          `sales_listpf&sid=${sid}&startDate=${startDate}&endDate=${endDate}`
        )
        .then(
    ...
    $scope.update_pd = function (item) {
      ...
      $rest.post("patch_pdpf", patch_obj).then(
        function success(res) {
    ...
    $scope.update_p = function (item) {
      let patch_obj = { id: item.tranrid };
      $rest.post("patch_pspf", patch_obj).then(
        function success(res) {
